...
| Active unit | M0.2 — Define the V0 trust contract |
| Unit status | `Ready for review` |
| Accepted V0 units | 1 / 35 |
| Last accepted unit | M0.1 — Scaffold the package |
| Next permitted work | Human review of M0.2; do not begin M0.3 |
| Stop point | Human review after M0.2; do not begin M0.3 |

## V0 milestone summary

| Milestone | Outcome | Units | Accepted | Status | Plan |
|---|---|---:|---:|---|---|
| M0 | Loadable extension skeleton and executable V0 specification | 3 | 1 | `In progress` | [M0 plan](./docs/milestones/M0_FOUNDATION.md) |
...
|---|---|---|---|---|---|
| M0.1 | Pi can load a deliberately inert Bun/TypeScript package skeleton and its checks run | `Accepted` | `a77e281` | `docs/learning/M0.1-package-scaffold.md` | PR #2 and evidence log |
| M0.2 | Trusted execution requires explicit opt-in and communicates shell-equivalent authority | `Ready for review` | — | `docs/learning/M0.2-trust-contract.md` | Evidence log; pending human review |
...
|---|---|---|---|---|---|---|
| 2026-07-31 | M0.1 | Accepted | `a77e281` | `bun install --frozen-lockfile` exit 0; `bun run typecheck` exit 0; `bun test` 3 pass; `bun run check` exit 0; `bun run smoke:pi` exit 0 (project-pinned Pi 0.80.10 loaded package root, answered RPC, and rejected a missing-extension control). Bun 1.3.11; host Pi 0.80.6 was recorded but is not the smoke runtime. | Project owner | Full verification was recorded on 2026-07-19; type, unit, and smoke checks were reconfirmed on 2026-07-31. Accepted by instruction to update progress and prepare M0.2; the complete scaffold was reviewed and merged through PR #2. |
| 2026-07-31 | M0.2 | Ready for review | — | Bun 1.3.11; project-pinned Pi 0.80.10; `bun install --frozen-lockfile` exit 0; `bun run typecheck` exit 0; `bun test` 7 pass; `bun run check` exit 0; `bun run smoke:pi` exit 0 (default path silent, trusted flag emitted the exact warning, missing-extension control rejected); `git diff --check` exit 0. | — | Consent contract only: no `code` tool, evaluator, product subprocess, IPC, or M0.3 fixture; stop for human review. |
